...
ETF_POOL = {
    ...
    "159901.SZ": {"name": "深100ETF", "category": "宽基指数"},

    # 行业ETF
    "512000.SH": {"name": "券商ETF", "category": "金融"},
    "512880.SH": {"name": "证券ETF", "category": "金融"},
    ...
    "159928.SZ": {"name": "消费ETF", "category": "消费"},

    # 军工
    "512660.SH": {"name": "军工ETF", "category": "军工"},
    "512670.SH": {"name": "国防ETF", "category": "军工"},
    ...
    "159922.SZ": {"name": "地产ETF", "category": "地产"},
}
...
def calculate_etf_score(fund_code, fund_data):
    ...
    if category in ['宽基指数', '医药', '消费', '策略']:
        score += 5
        details.append("稳健板块")
    elif category in ['军工', '资源']:
        score -= 5
