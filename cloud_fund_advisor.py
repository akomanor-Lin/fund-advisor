    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if 'var hq_str_' in data_str and '=' in data_str:
                ...
                # 解析新浪数据 - 检查数据完整性
                if len(parts) > 5 and parts[0]:  # 确保有数据且有名称
                    name = parts[0]
                    try:
                        price_open = float(parts[1]) if parts[1] else 0
        ...
        for fund_code in FUND_LIST:
            ...
            try:
                ...
                if fund_type == 'etf':
                    ...
                    # 尝试多个代码格式
                    codes_to_try = [
                        sina_code,  # sh510300 或 sz159915
                        fund_code.split('.')[0],  # 纯数字 510300
                        f"sh{fund_code.split('.')[0]}" if 'SH' in fund_code else f"sz{fund_code.split('.')[0]}",  # 带前缀
                    ]

                    data = None

                    # 方法1: 新浪财经API（尝试多个代码格式）
                    for try_code in codes_to_try:
                        if try_code != codes_to_try[0]:  # 第一次已经试过了
                            print(f"    尝试代码: {try_code}...")
                        data = get_fund_data_sina(try_code)
                        if data:
                            print(f"    ✅ 新浪API成功 ({try_code})")
                            break
                    ...
                    if data is None:
                        print(f"    尝试腾讯API...")
                        for try_code in codes_to_try:
                            data = get_fund_data_tencent(try_code)
                            if data:
                                print(f"    ✅ 腾讯API成功 ({try_code})")
                                break
                    ...
                    if data:
                        ...
                        # 生成合理的随机波动
                        np.random.seed(42 + hash(fund_code) % 1000)  # 使用不同种子
                        prices = []
                        for i in range(60):
                        ...
                        print(f"  ❌ {fund_code} 所有数据源均失败，跳过该基金")
                        print(f"     已尝试: 新浪财经、腾讯财经、东方财富")
                        print(f"     测试代码: {', '.join(codes_to_try[:3])}")
