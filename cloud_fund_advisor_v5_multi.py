...
def calculate_v5_score(fund_code, fund_data, position_info=None):
    ...
    if position_info and position_info.get('week52_high'):
...
def generate_position_report(scan_results, analysis):
    ...
    # 待建仓标的
    pending_positions = [p for p in scan_results if p['status'] == 'pending']
    if pending_positions:
        report_lines.append("⏳ 待建仓标的")
        report_lines.append("━" * 60)
        report_lines.append("")

        for p in pending_positions:
            position = next(pos for pos in MULTI_POSITIONS['positions'] if pos['fund_code'] == p['fund_code'])
            report_lines.append(f"【{p['fund_name']}】")
            report_lines.append(f"  计划金额: {position['principal']}元")
            report_lines.append(f"  当前价格: {p['current_price']:.3f}")
            report_lines.append(f"  V5评分: {p['v5_score']:.0f} - {p['recommendation']}")
            report_lines.append(f"  建议: {p['action']}")
            report_lines.append("")
    ...
    # 第二批建仓建议
    if pending_positions:
        pending_avg_score = sum(p['v5_score'] for p in pending_positions) / len(pending_positions)
        if pending_avg_score >= 70:
            report_lines.append("🚀 第二批建仓建议:")
            report_lines.append(f"  待建仓标的平均V5评分: {pending_avg_score:.0f}")
            report_lines.append("  建议考虑建仓消费/创业板ETF")
            report_lines.append("")
